...
import {
  Container, Avatar, Box, Typography, IconButton, Popover, Tooltip, TextField, Button, Modal, Paper
} from '@mui/material';
...
function AccountElm({
  index, edit, website, login, pass, category, mainpass
}) {
  const intl = useIntl();
        ...
        .then((res) => {
          if (res.success) {
            window.fetch(URL, {
              ...
              body: JSON.stringify({
                type: 'add', website: websiteField, login: window.btoa(loginField), password: window.btoa(encryptFn(passField, mainpass)), category: categoryField
              })
            }).then((res) => res.json())
              .then((res) => {
                if (res.success) {
  ...
  return (
    <>
      <Paper
        elevation={8}
        sx={{
          p: 1, py: 2, m: [1, 2], textAlign: 'left', display: 'flex', alignItems: 'center', width: ['90%', '330px']
        }}
      >
...
function Passes({ data, pass }) {
  ...
  const filteredData = data.filter((item) => Object.keys(item).some((key) => typeof item[key] === 'string' && item[key].toLowerCase().includes(query.toLowerCase())));
  useEffect(() => {
    if (data.length > 0) {
  ...
  return (
    <>
        ...
        <Box sx={{ m: 1 }}>
          {!edit ? (
            <Button
              ...
              sx={{
                maxWidth: '240px', marginLeft: 'auto', marginRight: 'auto', mt: 1
              }}
              startIcon={<SettingsIcon />}
              variant="contained"
            >
              {intl.formatMessage(messages.managePasses)}
            </Button>
          ) : (
            <Button
              color="error"
      ...
      {!err && (
      <Container sx={{
        textAlign: 'center', display: 'flex', flexWrap: 'wrap', justifyContent: 'center', mb: 5, mt: 1
      }}
      >
        {filteredData.map((elm) => (
          elm.index && elm.website && elm.login && elm.password && (
            <AccountElm
              ...
              website={elm.website}
              login={elm.login}
              pass={decryptFn(elm.password, pass)}
