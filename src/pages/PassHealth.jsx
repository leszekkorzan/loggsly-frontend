...
import {
  Container, Avatar, Box, Typography, Button, Paper
} from '@mui/material';
...
function HealthElm({
  ...
  return (
    <Paper
      key={index}
      elevation={8}
      style={{ border: leaked && '2px solid #f44336' }}
      sx={{
        p: 1, py: 2, m: [1, 2], textAlign: 'left', display: 'flex', alignItems: 'center', width: ['95%', '310px']
      }}
    >
      ...
      <Box sx={{ marginLeft: 'auto', width: '70px', paddingLeft: '5px' }}>
        ...
        {leaked && (
          <Typography sx={{
            fontSize: '14px', color: '#f44336', fontWeight: 'bold', textAlign: 'center'
          }}
          >
            {intl.formatMessage(messages.leaked)}
          </Typography>
        )}
      </Box>
  ...
  return (
    <Container sx={{ color: '#000', textAlign: 'center', pt: 5 }}>
      ...
      {show ? (
        ...
        >
          {data.map((elm, index) => <HealthElm key={index} website={elm.website} login={elm.login} pass={decryptFn(elm.password, window.atob(window.sessionStorage.getItem('pass')))} />)}
