...
import {
  Container, Typography, Paper, Slider, FormControlLabel, Checkbox, Box, TextField, Button
} from '@mui/material';
    ...
    >
      ...
      <Paper sx={{ p: 2, textAlign: 'left' }}>
        ...
        <Box>
          <FormControlLabel
            sx={{ mr: [2, 5] }}
            control={<Checkbox checked={isNum} onClick={() => setIsNum(!isNum)} />}
            label={intl.formatMessage(messages.digits)}
          />
          <FormControlLabel
            sx={{ mr: [2, 5] }}
            control={<Checkbox checked={isSym} onClick={() => setIsSym(!isSym)} />}
            label={intl.formatMessage(messages.symbols)}
          />
          <FormControlLabel
            sx={{ mr: [2, 5] }}
            control={
              <Checkbox checked={isLowerCase} onClick={() => setIsLowerCase(!isLowerCase)} />
            }
            label={intl.formatMessage(messages.lowerC)}
          />
          <FormControlLabel
            control={
              <Checkbox checked={isUpperCase} onClick={() => setIsUpperCase(!isUpperCase)} />
            }
            label={intl.formatMessage(messages.upperC)}
          />
        </Box>
      </Paper>
    </Container>
