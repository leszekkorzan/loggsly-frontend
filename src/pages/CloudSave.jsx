...
function ProfileElm({
  ...
  const removeProfile = () => {
    if (window.confirm(intl.formatMessage(messages.delete))) {
      window.sessionStorage.clear();
      remove(ref(db, `users/${userUID}/data/${Object.keys(dataDb)[index]}`)).then(() => {
...
function CloudSave() {
  ...
  const url = 'https://loggsly.herokuapp.com';
  ...
  return (
    <Container
      maxWidth="sm"
      sx={{
        marginLeft: 'auto', marginRight: 'auto', textAlign: 'center', color: '#000', mt: 5
      }}
    >
      {!data && !loading && (
        <Paper sx={{ p: 2 }}>
                ...
                {!edit ? (
                  ...
                  <Box>
                    <Accordion sx={{ mb: 2 }}>
                      ...
                      <AccordionDetails>
                        <Box sx={{
                          display: 'flex', flexWrap: 'wrap', justifyContent: 'center', flexDirection: ['column', 'row']
                        }}
                        >
                          <TextField
                            value={addingName}
                            ...
                            label={intl.formatMessage(messages.profileName)}
                            sx={{ m: 1 }}
                          />
                          <TextField
                            value={addingCSV}
                            onChange={(e) => setAddingCSV(e.target.value)}
                            disabled={addingLoading}
                            label={intl.formatMessage(messages.csvApiUrl)}
                            sx={{ m: 1 }}
                          />
                          <TextField
                            value={addingPass}
                            onChange={(e) => setAddingPass(e.target.value)}
                            disabled={addingLoading}
                            type="password"
                            label={intl.formatMessage(messages.masterPass)}
                            sx={{ m: 1 }}
                          />
                          <TextField
                            value={addingPass2}
                            onChange={(e) => setAddingPass2(e.target.value)}
                            disabled={addingLoading}
                            type="password"
                            label={intl.formatMessage(messages.repeatMasterPass)}
                            sx={{ m: 1 }}
                          />
                          <Button
                            onClick={addProfile}
