...
import {
  Container, Typography, TextField, Button, Divider, Alert, AlertTitle, Paper
} from '@mui/material';
  ...
  const addPass = () => {
    if (window.localStorage.getItem('manage_api') !== null && website.length > 0 && login.length > 0 && password.length > 0) {
      ...
      window.fetch(URL, {
        ...
        body: JSON.stringify({
          type: 'add', website, login: window.btoa(login), password: window.btoa(encryptFn(password, pass)), category
        })
      }).then((res) => res.json())
        .then((res) => {
          if (res.success) {
      ...
      <Container sx={{ mt: 5, color: '#000', textAlign: 'center' }}>
        ...
        {window.localStorage.getItem('manage_api') !== null ? (
          <div>
            {!success ? (
              <Paper sx={{
                maxWidth: '400px', p: 2, marginLeft: 'auto', marginRight: 'auto'
              }}
              >
              ...
              <>
                <Alert
                  sx={{
                    textAlign: 'left', m: 2, maxWidth: '300px', marginLeft: 'auto', marginRight: 'auto'
                  }}
                  severity="success"
                >
        ...
        <Paper sx={{
          p: [1, 2], mt: 2, ml: 'auto', mr: 'auto', maxWidth: '400px'
        }}
        >
